...
class EmailManager:
    ...
    IMPORTANT_TEMPLATES = [
        'withdraw_request_confirmation_code', 'new_device_notif', 'tfa_enable_notif',
    ]
    CRITICAL_TEMPLATES = [
        'welcome', 'template', 'otp', 'reset_password', 'tfa_otp', 'tfa_removal', 'giftcard',
        'change_password_notif', 'change_mobile_notif', 'change_email_notif',
    ]
    ...
    def send_mail_many(cls, mail_kwargs: List[dict]):
        ...
        is_logging_enabled = NotificationConfig.is_email_logging_enabled()
        is_broker_enabled = NotificationConfig.is_email_broker_enabled()

        if is_logging_enabled or is_broker_enabled:
            for kw in mail_kwargs:
                for schema in cls._build_schemas_from_mail_kw(kw):
                    notification_producer.write_event(Topics.EMAIL.value, schema.serialize())
    ...
    def send_withdraw_request_confirmation_code(cls, withdraw_request):
        ...
        if user.is_email_verified:
            ...
            send_withdraw_email_otp.apply_async(
                ...
                countdown=int(Settings.get_value('withdraw_otp_email_task_countdown', default=30))
                if user.has_verified_mobile_number
                else 0,  # Send immediately if mobile is not verified
                expires=10 * 60,
            )
        ...
        if emergency_cancel_url:
            telegram_message += '\n\n*لغو اضطراری:* ' + emergency_cancel_url
        Notification(user=user, message=telegram_message.format(
            amount_str, target,
    ...
    @classmethod
    def _build_schemas_from_mail_kw(cls, mail_kw: dict) -> List[EmailSchema]:
        """
        Helper to convert mail_kwargs (dict) into one or more EmailSchema objects.
        Expands both recipients and optional bcc into separate EmailSchema entries.
        """
        schemas = []
        to_field = mail_kw['recipients']
        if not isinstance(to_field, list):
            to_field = [to_field]

        bcc_field = mail_kw.get('bcc', [])
        if not isinstance(bcc_field, list):
            bcc_field = [bcc_field] if bcc_field else []

        for rcp in to_field + bcc_field:
            schemas.append(
                EmailSchema(
                    to=str(rcp),
                    template=mail_kw.get('template'),
                    context=mail_kw.get('context', {}),
                    priority=mail_kw.get('priority'),
                    backend=mail_kw.get('backend'),
                    scheduled_time=mail_kw.get('scheduled_time'),
                )
            )
        return schemas
