...
from exchange.notification.fcm import send_batch_fcm_notifications
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--delay_time',
            nargs='?',
            default=5 if settings.IS_PROD else 10,
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        while True:
            if not NotificationConfig.is_notification_broker_enabled():
                self.stdout.write(
                    self.style.SUCCESS('Notification broker is not active, Sending Notifications here is disabled.')
                )
