...
class SmsSender:
    ...
    def send_with_finnotext(cls, sms):
        ...
        try:
            ...
            metric_incr(
                f'metric_sms_provider_api_counter__finnotext_SuccessfulSend_{get_human_readable_template_name(sms.template)}'
            )
        except ConnectionFailedException:
            details = f'Failed: {track_id} connection failed'
            delivery_status = 'Sent: Unsuccessful'
            metric_incr(
                f'metric_sms_provider_api_counter__finnotext_ConnectionFailed_{get_human_readable_template_name(sms.template)}'
            )
        ...
        sms.carrier = Sms.CARRIERS.finnotext
...
class OldSmsIrClient:
    ...
    def send(cls, sms: 'Sms'):
        ...
        with measure_time_cm(metric='metric_sms_provider_api_process_time__oldsmsir_Token'):
            r = requests.post(
        ...
        if fast_send:
            with measure_time_cm(metric='metric_sms_provider_api_process_time__oldsmsir_UltraFastSend'):
                r = requests.post(
            ...
            text = sms.text + '\n' + 'لغو 11'
            with measure_time_cm(metric='metric_sms_provider_api_process_time__oldsmsir_MessageSend'):
                r = requests.post(
        ...
        try:
            ...
            metric_incr(
                f'metric_sms_provider_api_counter__oldsmsir_{error}_{get_human_readable_template_name(sms.template)}'
            )
            raise e
        ...
        if not r['IsSuccessful']:
            metric_incr(
                f'metric_sms_provider_api_counter__oldsmsir_UnsuccessfulSend_{get_human_readable_template_name(sms.template)}'
            )
        ...
        metric_incr(
            f'metric_sms_provider_api_counter__oldsmsir_SuccessfulSend_{get_human_readable_template_name(sms.template)}'
        )
...
class NewSmsIrClient:
    ...
    def send(cls, sms: 'Sms'):
        ...
        with measure_time_cm(metric='metric_sms_provider_api_process_time__newsmsir_Verify'):
            r = requests.post(
                cls.URL,
                json={
        ...
        try:
            ...
            error = e.__class__.__name__
            metric_incr(
                f'metric_sms_provider_api_counter__newsmsir_{error}_{get_human_readable_template_name(sms.template)}'
            )
            raise e
        ...
        if int(r['status']) not in cls.SUCCESSFUL_STATUSES:
            ...
            metric_incr(
                f'metric_sms_provider_api_counter__newsmsir_UnsuccessfulSend_{get_human_readable_template_name(sms.template)}'
            )
            sms.details = (
                'Failed:'
                + (
            ...
            sms.delivery_status = 'Sent: Unsuccessful'
            sms.carrier = Sms.CARRIERS.new_restfulsms
            sms.save(update_fields=['details', 'delivery_status', 'carrier'])
            return
        ...
        metric_incr(
            f'metric_sms_provider_api_counter__newsmsir_SuccessfulSend_{get_human_readable_template_name(sms.template)}'
        )
...
class KavenegarClient:
    ...
    def send(cls, sms: 'Sms'):
        ...
        with measure_time_cm(metric='metric_sms_provider_api_process_time__kavenegar_Verify'):
        ...
        try:
            ...
            error = e.__class__.__name__
            metric_incr(
                f'metric_sms_provider_api_counter__kavenegar_{error}_{get_human_readable_template_name(sms.template)}'
            )
            raise e
        ...
        if int(status_code) not in cls.SUCCESSFUL_STATUSES:
            ...
            metric_incr(
                f'metric_sms_provider_api_counter__kavenegar_UnsuccessfulSend_{get_human_readable_template_name(sms.template)}'
            )
            sms.details = ('Failed: ' + status_message)[:100]
            sms.delivery_status = 'Sent: Unsuccessful'
            sms.carrier = Sms.CARRIERS.kavenegar
            sms.save(update_fields=['details', 'delivery_status', 'carrier'])
            return
        ...
        metric_incr(
            f'metric_sms_provider_api_counter__kavenegar_SuccessfulSend_{get_human_readable_template_name(sms.template)}'
        )
