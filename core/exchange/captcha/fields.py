...
class BaseCaptchaTextInput(MultiWidget):
    ...
    def fetch_captcha_store(self, name, value, attrs=None, generator=None):
        ...
        key = CaptchaStore.pick() if settings.CAPTCHA_GET_FROM_POOL else CaptchaStore.generate_key(generator)
...
class CaptchaTextInput(BaseCaptchaTextInput):
        ...
        if self.output_format:
            for key in ('image', 'hidden_field', 'text_field'):
                if '%%(%s)s' % key not in self.output_format:
                    msg = 'All of {} must be present in your CAPTCHA_OUTPUT_FORMAT setting. Could not find {}'.format(
    ...
    def build_attrs(self, *args, **kwargs):
        ...
        if self.id_prefix and 'id' in ret:
            ret['id'] = '{}_{}'.format(self.id_prefix, ret['id'])
        return ret

    def id_for_label(self, id_):
        ret = super().id_for_label(id_)
        if self.id_prefix and 'id' in ret:
            ret = f'{self.id_prefix}_{ret}'
        return ret
    ...
    def _direct_render(self, name, attrs):
        ...
        context = {
            ...
            'name': name,
            'key': self._key,
            'id': '{}_{}'.format(self.id_prefix, attrs.get('id')) if self.id_prefix else attrs.get('id'),
        ...
        if settings.CAPTCHA_TEST_MODE and response.lower() == 'passed':
            ...
            try:
                ...
                raise ValidationError(
                    getattr(self, 'error_messages', {}).get('invalid', gettext_lazy('Invalid CAPTCHA')),
                )
        return value
