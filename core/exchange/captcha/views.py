...
from exchange.captcha.helpers import captcha_audio_url, captcha_image_url, has_farsi_chars
...
def captcha_image(request, key, scale=1):
    ...
    rotation = settings.CAPTCHA_LETTER_ROTATION
    _has_farsi_chars = has_farsi_chars(text)

    font_path = settings.get_font_path()
    if _has_farsi_chars:
        fontpath = settings.CAPTCHA_FARSI_FONT
        text = text[::-1]
        rotation = (-3, 3)
    elif isinstance(font_path, str):
        fontpath = font_path
    elif isinstance(font_path, (list, tuple)):
        fontpath = random.choice(font_path)
    else:
...
def captcha_audio(request, key):
    if settings.CAPTCHA_FLITE_PATH:
        ...
        text = store.challenge
        if settings.get_challenge() == 'captcha.helpers.math_challenge':
