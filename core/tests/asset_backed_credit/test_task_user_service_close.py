...
from exchange.asset_backed_credit.tasks import task_close_user_service


class TestTaskCloseUserService(TestCase):
    @override_settings(CELERY_TASK_ALWAYS_EAGER=True)
    @mock.patch(
        'exchange.asset_backed_credit.externals.providers.tara.TaraCheckUserBalance.request',
        lambda _: Decimal(1_000_000_0),
    )
    @mock.patch('exchange.asset_backed_credit.externals.providers.tara.TaraDischargeAccount.request', lambda _: '12345')
    @mock.patch('exchange.asset_backed_credit.tasks.remove_user_restriction_task.delay')
    def test_success(self, mock_restriction_task):
        ...
        result = task_close_user_service.delay(service_ids).result

        for service_id in service_ids[0:3]:
            assert result[service_id]['status'] == 'success'
            assert result[service_id]['message'] == 'user service closed'
