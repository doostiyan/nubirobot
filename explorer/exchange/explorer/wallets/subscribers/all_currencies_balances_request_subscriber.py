...
from exchange.explorer.wallets.subscribers.dto.get_all_currencies_balances_request import \
    GetAllCurrenciesBalancesRequest
from exchange.explorer.wallets.subscribers.dto.get_balances_response import GetBalancesResponse
from exchange.explorer.wallets.subscribers.topics import GET_ALL_CURRENCIES_BALANCES_REQUEST_QUEUE, \
    GET_ALL_CURRENCIES_BALANCES_RESPONSE_ROUTING_KEY
from exchange.explorer.wallets.utils.provider_api import ProviderApiUtils


class AllCurrenciesBalancesRequestSubscriber(SubscriberInterface):
    ...
    def callback(self, message: str) -> None:
        self.logger.info(f"All Currencies: Received message: {message}")
        try:
            # Validate and parse the incoming message
            get_all_currencies_balances_request =   GetAllCurrenciesBalancesRequest.model_validate_json(message)

            # Load the API for the specified network
            api = ProviderApiUtils.load_provider_api(get_all_currencies_balances_request.network, Operation.BALANCE)

            # Fetch balances for the requested addresses
            balance_updated_addresses = ExplorerInterface.get_all_tokens_balances_standalone(
                api,
                get_all_currencies_balances_request.addresses
            )
            if not balance_updated_addresses or len(balance_updated_addresses) == 0:
                self.logger.info("All Currencies: No balances updated. Skipping further processing")
                return

            # Compare the input addresses with the result addresses
            input_addresses = {address.lower() for address in get_all_currencies_balances_request.addresses}
            ...
            if unmatched_input_addresses:
                error_message = (
                    f"All Currencies: Addresses in input but not in results: {unmatched_input_addresses}"
                )
            ...
            self.logger.info(
                f"All Currencies: Published balance updates to routing key: {GET_ALL_CURRENCIES_BALANCES_RESPONSE_ROUTING_KEY}")

        except Exception as e:
            # Log the error and prevent the callback from crashing
            self.logger.error(f"All Currencies: An error occurred while processing the message: {str(e)}",
                              exc_info=True)
            raise e
