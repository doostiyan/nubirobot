...
from exchange.explorer.wallets.subscribers.dto.get_all_currencies_balances_request import GetAllCurrenciesBalancesRequest
...
from exchange.explorer.wallets.tests.fixtures.all_currencies_balances_subscriber_fixture import (
...
def test__get_topic__returns_correct_queue(subscriber):
    ...
    assert subscriber.topic() == GET_ALL_CURRENCIES_BALANCES_REQUEST_QUEUE

def test__callback__when_valid_request__publishes_balances(subscriber, batch_request_data, batch_balances_result):
    # Mock the necessary dependencies
    with patch('exchange.explorer.wallets.utils.provider_api.ProviderApiUtils.load_provider_api') as mock_load_api, \
         patch('exchange.blockchain.api.general.explorer_interface.ExplorerInterface.get_all_tokens_balances_standalone') as mock_get_balances:
        ...
        request = GetAllCurrenciesBalancesRequest(**batch_request_data)
        ...
        # Verify routing key
        from exchange.explorer.wallets.subscribers.topics import GET_ALL_CURRENCIES_BALANCES_RESPONSE_ROUTING_KEY
        assert call_args[0] == GET_ALL_CURRENCIES_BALANCES_RESPONSE_ROUTING_KEY
...
def test__callback__when_mismatched_addresses__raises_value_error(subscriber, batch_request_data):
    with patch('exchange.explorer.wallets.utils.provider_api.ProviderApiUtils.load_provider_api') as mock_load_api, \
        ...
        assert "All Currencies: Addresses in input but not in results" in str(exc_info.value)
        subscriber._message_broker.publish.assert_not_called() 