...
def ada_bech32_decode(bech: str, *, ignore_long_length=False) -> DecodedBech32:
    """Validate a Bech32/Bech32m string, and determine HRP and data."""
    bech_lower = bech.lower()
    if bech_lower != bech and bech.upper() != bech:
        return DecodedBech32(None, None, None)
...
def one_to_eth_address(one_address):
    _, hrp, data = bech32_decode(one_address)
    buf = convertbits(data, 5, 8, False)
    return '0x' + ''.join('{:02x}'.format(x) for x in buf)


def eth_to_one_address(eth_address):
    hrp = 'one'
    enc = Encoding.BECH32
    eth_address = eth_address[2:]
    data = binascii.a2b_hex(eth_address)
    data = convertbits(data, 8, 5)
    return bech32_encode(enc, hrp, data)
