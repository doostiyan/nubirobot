import pytest
from exchange.blockchain.api.algorand.algorand_algonode import BlockDaemonAlgorandApi
from exchange.blockchain.tests.fixtures.algo_algonode_fixtures import tx_hash_successful, tx_hash_fail, address, from_block, to_block


api = BlockDaemonAlgorandApi
...
@pytest.mark.slow
def test__block_daemon_algo__get_balance__api_call__successful(address):
...
@pytest.mark.slow
def test__block_daemon_algo__get_address_txs__api_call__successful(address):
...
@pytest.mark.slow
def test__block_daemon_algo__get_block_head__api_call__successful():
...
@pytest.mark.slow
def test__block_daemon_algo__get_tx_details__api_call__successful(tx_hash_successful, tx_hash_fail):
    tx_details_keys = {'transaction', 'current-round'}
    transaction_keys = {'close-rewards', 'closing-amount', 'confirmed-round', 'fee', 'first-valid', 'genesis-hash', 'genesis-id', 'id', 'intra-round-offset', 'last-valid', 'payment-transaction', 'receiver-rewards', 'round-time', 'sender', 'sender-rewards', 'signature', 'tx-type'}
    payment_transaction_keys = {'amount','close-amount', 'receiver'}
...
@pytest.mark.slow
def test__block_daemon_algo__get_blocks_txs__api_call__successful(from_block, to_block):
