...
from typing import Optional, Type, Dict, Any, Union, Sequence, Tuple

from exchange.blockchain import constants, segwit_address
from exchange.blockchain.constants import AbstractNet, bh2u
from exchange.blockchain.segwit_address import bech32_encode, bech32_decode, CHARSET, convertbits, CHARSET_INVERSE
...
class BaseDecodeError(Exception): pass
class InvalidChecksum(BaseDecodeError):pass
...
def encode_fallback_addr(fallback: str, net: Type[AbstractNet]) -> Sequence[int]:
    ...
    wver, wprog_ints = segwit_address.decode_segwit_address(net.SEGWIT_HRP, fallback)
    if wver is not None:
...
def parse_fallback_addr(data5: Sequence[int], net: Type[AbstractNet]) -> Optional[str]:
    ...
    if wver == 17:
        ...
        addr = segwit_address.encode_segwit_address(net.SEGWIT_HRP, wver, data8)
    else:
        return None
    return addr
...
def lnencode(addr: 'LnAddr', privkey) -> str:
    from exchange.blockchain import ecc
    if addr.amount:
    ...
    return bech32_encode(segwit_address.Encoding.BECH32, hrp, data5)
...
def lndecode(invoice: str, *, verbose=False, net=None) -> LnAddr:
    ...
    if decoded_bech32.encoding != segwit_address.Encoding.BECH32:
