...
use winterfell::{FieldExtension, StarkProof};
...
impl WorkProver {
    pub fn verify_work(start: BaseElement, result: BaseElement, proof: StarkProof) {
        ...
        match winterfell::verify::<
            WorkAir,
            Blake3_256<<Self as Prover>::BaseField>,
            DefaultRandomCoin<Blake3_256<<Self as Prover>::BaseField>>,
        >(proof, pub_inputs)
        {
...
fn main() {
    ...
    let start = BaseElement::new(3);
    WorkProver::verify_work(start, result, proof);
